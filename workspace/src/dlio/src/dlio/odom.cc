...
dlio::ESKF<float> eskf_filter;
dlio::ESKF<float> meas_filter;
std::vector<dlio::ImuMeas> imuBank;
dlio::OdomNode::OdomNode(ros::NodeHandle node_handle) : nh(node_handle) {
  ...
  this->odom_base_pub  = this->nh.advertise<nav_msgs::Odometry>("odom_base", 1, true);
  ...
  this->arrow_pub = this->nh.advertise<visualization_msgs::Marker>("normals", 1, true);
  ...
  eskf_filter = ESKF<float>();
  meas_filter = ESKF<float>();
...
void dlio::OdomNode::getParams() {
  ...
  if (!this->imu_calibrate_) {
    ...
    eskf_filter.SetInitialConditions(ESKF<float>::Options(),Eigen::Vector3f(0,0,0),Eigen::Vector3f(0,0,0));
    meas_filter.SetInitialConditions(ESKF<float>::Options(),Eigen::Vector3f(0,0,0),Eigen::Vector3f(0,0,0));
  }
...
void dlio::OdomNode::getScanFromROS(const livox_ros_driver2::CustomMsg::ConstPtr& pc)
{
    pcl::PointCloud<PointType>::Ptr original_scan_ (boost::make_shared<pcl::PointCloud<PointType>>());
    
    int plsize = pc->point_num;
    int valid_num = 0;
    int point_filter_num = 1;

    pcl::PointCloud<PointType> pointBuf;
    pointBuf.clear();
    pointBuf.reserve(plsize);


    for(uint i=0; i<plsize; i++)
    {
      if((pc->points[i].line < 4) && ((pc->points[i].tag & 0x30) == 0x10 || (pc->points[i].tag & 0x30) == 0x00))
      {
        ...
        if (valid_num % point_filter_num == 0)
        {
          ...
          _p.y = pc->points[i].y;
          _p.z = pc->points[i].z;
          _p.intensity = pc->points[i].reflectivity;
          _p.timestamp = pc->points[i].offset_time; // use curvature as time of each laser points, curvature unit: ms
          pointBuf.points.push_back(_p);
          // std::cout<<"timestamp "<< pc->points[i].offset_time <<std::endl;

          // if(((abs(pointBuf[i].x - pointBuf[i-1].x) > 1e-7) 
          //     || (abs(pointBuf[i].y - pointBuf[i-1].y) > 1e-7)
          //     || (abs(pointBuf[i].z - pointBuf[i-1].z) > 1e-7))
          //   )
          // {
    ...
    *original_scan_ = pointBuf;
    this->deskew_ = true;

    std::vector<int> idx;
    original_scan_->is_dense = false;
    original_scan_->width = valid_num;
...
void dlio::OdomNode::deskewPointcloud() {
  ...
  if (frames.size() != timestamps.size()) {
    // ERROR("Bad time sync between LiDAR and IMU!");
...
void dlio::OdomNode::callbackPointCloud(const livox_ros_driver2::CustomMsg::ConstPtr &pc)
{
  loop_cnt++;
  ...
  if (this->current_scan->points.size() <= this->gicp_min_num_points_) {
    ...
    // ERROR("Low number of points in current scan, number:  {0}", this->current_scan->points.size());
    return;
  }
...
#ifdef CAL_DEGEN_DIRECTION
  this->concave_hull.setInputCloud(this->current_scan);
  ...
  // INFO("convex size {}", concave_points->size());

  pcl::NormalEstimation<PointType,pcl::Normal> normalEstimation;
  
  for(size_t i=0; i < concave_points->size(); i++)
  {
    PointType pt = concave_points->points[i];
    std::vector<int> k_indices;
    std::vector<float> k_sq_dists;
    this->gicp.source_kdtree_->nearestKSearch(pt, 5, k_indices, k_sq_dists);

    pcl::PointCloud<PointType>::Ptr pointsAroundConvex =
      pcl::PointCloud<PointType>::Ptr (boost::make_shared<pcl::PointCloud<PointType>>());
    
    pointsAroundConvex->push_back(pt);
    for(size_t j=0;j<k_indices.size();j++)
      pointsAroundConvex->push_back(this->current_scan->points[k_indices[j]]);

    normalEstimation.setInputCloud(pointsAroundConvex);
    ...
    Eigen::Vector3f ang_normal(0,0,0);

    for(size_t p=0;p<normals->size();p++)
    {
      ang_normal.x()+=normals->points[p].normal_x;
      ang_normal.y()+=normals->points[p].normal_y;
      ang_normal.z()+=normals->points[p].normal_z;
    }
    ang_normal/=normals->size();

    visualization_msgs::Marker normal_msg;

    normal_msg.header.stamp = pc->header.stamp;
    normal_msg.header.frame_id = this->odom_frame;
    normal_msg.ns = "convex_normal";
    normal_msg.id = i;
    normal_msg.type = visualization_msgs::Marker::ARROW;

    normal_msg.pose.position.x = pt.x;
    normal_msg.pose.position.y = pt.y;
    normal_msg.pose.position.z = pt.z;

    
    Eigen::Quaternionf quat;

    // // Set the real part of the quaternion to 0
    // quat.w() = 0;

    // // Set the imaginary parts of the quaternion to the components of the unit vector
    // quat.x() = normals->points[0].normal_x;
    // quat.y() = normals->points[0].normal_y;
    // quat.z() = normals->points[0].normal_z;
    // quat.normalize();

    quat.setFromTwoVectors(Eigen::Vector3f::UnitX(), Eigen::Vector3f(ang_normal.x(), ang_normal.y(), ang_normal.z()));
    quat.normalize();

    normal_msg.pose.orientation.w = quat.w();
    normal_msg.pose.orientation.x = quat.x();
    normal_msg.pose.orientation.y = quat.y();
    normal_msg.pose.orientation.z = quat.z();

    normal_msg.scale.x = 1;
    normal_msg.scale.y = 0.2;
    normal_msg.scale.z = 0.2;

    normal_msg.color.r = 0.0;
    normal_msg.color.g = 0.0;
    normal_msg.color.b = 1.0;
    normal_msg.color.a = 1.0;

    normal_msg.lifetime= ros::Duration(0.14);

    arrow_pub.publish(normal_msg);


    // INFO("normal {0} {1} {2}", normals->points[0].normal_x, normals->points[0].normal_y, normals->points[0].normal_z);
  }
#endif
  auto convex_point = std::chrono::high_resolution_clock::now();
  ...
  //do propagate here
  Eigen::Vector3f avg_w(0,0,0);

  for(size_t i=0;i<imuBank.size();i++)
  {
      meas_filter.Predict(imuBank[i]);
      avg_w +=imuBank[i].ang_vel;
  }
  avg_w = avg_w/imuBank.size();

  Sophus::SO3f SO3_R = Sophus::SO3<float>(this->lidarPose.q.normalized().toRotationMatrix());
  Sophus::SE3<float> lidarSE3(SO3_R, this->lidarPose.p);

  Sophus::SO3f imuRot = Sophus::SO3<float>(meas_filter.GetNominalSE3().so3().matrix());
  Sophus::SO3f diffRot = SO3_R * imuRot.inverse();

  Eigen::Vector3f diffPos = this->lidarPose.p - meas_filter.GetNominalSE3().translation();

  bad_lidar_cnt ++;

  Eigen::Matrix3f transRot;

  transRot<<1.0,  0.0000000,  0, 
            0.0000000,  1.0000000,  0.0000000, 
            0,  0.0000000,  1.0;
  Eigen::Vector3f curVelInB = transRot.inverse() * meas_filter.GetNominalSE3().so3().matrix().inverse() * meas_filter.GetNominalVel() ;
  // INFO("diff pos orientation {0} {1} {2}", diffPosInB.x()/diffPosInB.norm(),diffPosInB.y()/diffPosInB.norm(),diffPosInB.z()/diffPosInB.norm());
  auto loop_end = std::chrono::high_resolution_clock::now();

  std::chrono::duration<double, std::milli> loopTimeCost = loop_end - loop_start;
  std::chrono::duration<double, std::milli> gicpTimeCost = gicp_end - gicp_start;
  std::chrono::duration<double, std::milli> start2gicp = gicp_start - loop_start;
  std::chrono::duration<double, std::milli> gicp2end = loop_end -  gicp_end;
  // INFO("current point number {0}", this->current_scan->points.size());
  // INFO("current position {0}, {1}, {2},", meas_filter.GetNominalSE3().translation().x(), meas_filter.GetNominalSE3().translation().y(), meas_filter.GetNominalSE3().translation().z());
  // INFO("current velocity {0}, {1}, {2},", curVelInB.x(), curVelInB.y(), curVelInB.z());
  // INFO("rot diff {0}, ", (diffRot.log().norm()/(1 + avg_w.norm())));
  // INFO("pos diff {0}, ", diffPos.norm());
  // INFO("acc bias {0}, {1}, {2},,  ", meas_filter.GetNominalBa().x(), meas_filter.GetNominalBa().y(), meas_filter.GetNominalBa().z());
  // INFO("gyro bias {0}, {1}, {2},, ", meas_filter.GetNominalBg().x(), meas_filter.GetNominalBg().y(), meas_filter.GetNominalBg().z());
  // INFO("current acc value {0}, {1}, {2}, sqrt {3}", imuBank.back().lin_accel.x(), imuBank.back().lin_accel.y(), imuBank.back().lin_accel.z(), imuBank.back().lin_accel.norm());
  // INFO("current gyro value {0}, {1}, {2}, sqrt {3}", imuBank.back().ang_vel.x(), imuBank.back().ang_vel.y(), imuBank.back().ang_vel.z(), imuBank.back().ang_vel.norm());
  // INFO("gravity {0}, {1}, {2},, ", meas_filter.GetGravity().x(), meas_filter.GetGravity().y(), meas_filter.GetGravity().z());
  // INFO("gicp time cost {0} ms,  loop cost {1} ms, debug 1 {2} ms  debug2 {3} ms  ", gicpTimeCost.count(), loopTimeCost.count(), start2gicp.count(), gicp2end.count());
  // INFO("----------------{0}th Loop Ends-----------  ", loop_cnt);
  ...
  {
    // ERROR("GICP not converged");
    lidar_unreliable = true;
    this->lidarPose.p = meas_filter.GetNominalSE3().translation();
    this->lidarPose.q = Eigen::Quaternionf(meas_filter.GetNominalSE3().so3().matrix());
    lidarSE3 = Sophus::SE3<float>(imuRot, this->lidarPose.p);
    this->T_corr = Eigen::Matrix4f::Identity();
    Eigen::Matrix4f newPose = Eigen::Matrix4f::Identity();
    newPose.block<3,3>(0,0) = meas_filter.GetNominalSE3().so3().matrix();
    newPose.block<3,1>(0,3) = meas_filter.GetNominalSE3().translation();
    this->T_corr = newPose * this->T_prior.inverse();
    this->force_update = true;
    // bad_lidar_cnt +=2;
  }
  else if((diffRot.log().norm()/(1 + avg_w.norm())>0.2&&diffRot.log().norm()/(1 + avg_w.norm())<0.5))
  {
    // ERROR("rotation drift norm {0}", diffRot.log().norm()/(1 + avg_w.norm()));
    lidar_unreliable = true;
    this->lidarPose.p = meas_filter.GetNominalSE3().translation();
    this->lidarPose.q = Eigen::Quaternionf(meas_filter.GetNominalSE3().so3().matrix());
    lidarSE3 = Sophus::SE3<float>(imuRot, this->lidarPose.p);
  ...
  // else if((diffPos.norm()>0.1 && diffPos.norm()< 0.5) && esks_converged == true)
  // {
  //   ERROR("translation drift norm {0}", diffPos.norm());
  //   // lidar_unreliable = true;
  //   // this->lidarPose.p = meas_filter.GetNominalSE3().translation();
  //   // this->lidarPose.q = Eigen::Quaternionf(meas_filter.GetNominalSE3().so3().matrix());
  //   // lidarSE3 = Sophus::SE3<float>(imuRot, this->lidarPose.p);
  ...
  //   // newPose.block<3,1>(0,3) = meas_filter.GetNominalSE3().translation();
  //   // this->T_corr = newPose * this->T_prior.inverse();
  //   // this->force_no_update = true;

  //   // use_imu_update = true;
  //   // esks_converged = false;
  //   // bad_lidar_cnt+=2;
  // }
  // else if(use_imu_update)
  // {
  //   ERROR("translation drift norm {0}", diffPos.norm());
  //   lidar_unreliable = true;
  //   this->lidarPose.p = meas_filter.GetNominalSE3().translation();
  ...
  //   newPose.block<3,1>(0,3) = meas_filter.GetNominalSE3().translation();
  //   this->T_corr = newPose * this->T_prior.inverse();
  //   use_imu_cooldown_cnt++;
  //   if(use_imu_cooldown_cnt>6)
  //   {
  //     use_imu_cooldown_cnt=0;
  ...
  else 
  {
    ...
    // bad_lidar_cnt--;
    // if(bad_lidar_cnt==0)
    meas_filter.ObserveSE3(lidarSE3, 0.1, 0.1 * M_PI / 180);
  }
  
  
  // std::cout << "norm rot "<< diffRot.log().norm()/(1 + avg_w.norm())<<"" << std::endl;
  if(diffPos.norm()<0.05)
    esks_converged= true;
  // if(diffPos.norm()>0.02)
  // // std::cout << "norm  pos  "<< diffPos.norm()<<"" << std::endl;

  {// to publish baselink 
  static tf2_ros::TransformBroadcaster br;
  geometry_msgs::TransformStamped transformStamped;

  transformStamped.header.stamp = pc->header.stamp;
  transformStamped.header.frame_id = this->odom_frame;
  transformStamped.child_frame_id = "/dlio/baselink";

  Eigen::Matrix4f origPose = Eigen::Matrix4f::Identity();
  origPose.block<3,3>(0,0) = meas_filter.GetNominalSE3().so3().matrix();
  origPose.block<3,1>(0,3) = meas_filter.GetNominalSE3().translation();

  Eigen::Matrix4f transMat;
  transMat<<1.0,  0.0000000,  0.0,  0.0,
            0.0000000,  1.0000000,  0.0000000,  0,
            0.0,  0.0000000,  1.0, -0.06,
            0.0,        0.0,        0.0,        1.0;
  
  Eigen::Matrix4f poseToPub =  origPose * transMat;

  Eigen::Quaternionf quatToPub(poseToPub.block<3,3>(0,0));
  Eigen::Vector3f posToPub = poseToPub.block<3,1>(0,3);


  transformStamped.transform.translation.x = posToPub.x();
  transformStamped.transform.translation.y = posToPub.y();
  transformStamped.transform.translation.z = posToPub.z();

  transformStamped.transform.rotation.w = quatToPub.w();
  transformStamped.transform.rotation.x = quatToPub.x();
  transformStamped.transform.rotation.y = quatToPub.y();
  transformStamped.transform.rotation.z = quatToPub.z();

  br.sendTransform(transformStamped);
  

  
  this->odom_base_ros.header.stamp = pc->header.stamp;
  this->odom_base_ros.header.frame_id = this->odom_frame;
  this->odom_base_ros.child_frame_id = "/dlio/base_odom";

  this->odom_base_ros.pose.pose.position.x = posToPub.x();
  this->odom_base_ros.pose.pose.position.y = posToPub.y();
  this->odom_base_ros.pose.pose.position.z = posToPub.z();

  this->odom_base_ros.pose.pose.orientation.w = quatToPub.w();
  this->odom_base_ros.pose.pose.orientation.x = quatToPub.x();
  this->odom_base_ros.pose.pose.orientation.y = quatToPub.y();
  this->odom_base_ros.pose.pose.orientation.z = quatToPub.z();

  this->odom_base_ros.twist.twist.linear.x = 0;
  this->odom_base_ros.twist.twist.linear.y = 0;
  this->odom_base_ros.twist.twist.linear.z = 0;

  this->odom_base_ros.twist.twist.angular.x = 0;
  this->odom_base_ros.twist.twist.angular.y = 0;
  this->odom_base_ros.twist.twist.angular.z = 0;

  this->odom_base_pub.publish(this->odom_base_ros);
  }
  ...
  // T_discrepency = newPose * this->T_prior.inverse();
  // use eskf to upgrade map
  ...
  // this->state.q = Eigen::Quaternionf(meas_filter.GetNominalSE3().so3().matrix());
  // this->state.p = meas_filter.GetNominalSE3().translation();
  // this->state.v.lin.w = meas_filter.GetNominalVel();
  ...
  // this->state.b.gyro = meas_filter.GetNominalBg();
  // this->state.b.accel = meas_filter.GetNominalBa();
  //
...
void dlio::OdomNode::callbackImu(const sensor_msgs::Imu::ConstPtr& imu_raw) {
  ...
  lin_accel[0] = imu->linear_acceleration.x * 9.80655;
  lin_accel[1] = imu->linear_acceleration.y * 9.80655;
  lin_accel[2] = imu->linear_acceleration.z * 9.80655;
...
void dlio::OdomNode::getNextPose() {
  ...
  // std::cout<<this->gicp.getFinalError()<<std::endl;
...
void dlio::OdomNode::propagateState() {
  ...
  eskf_filter.Predict(this->imu_meas);
  this->state.q = Eigen::Quaternionf(eskf_filter.GetNominalSE3().so3().matrix());
  this->state.p = eskf_filter.GetNominalSE3().translation();
  this->state.v.ang.b = this->imu_meas.ang_vel;
  this->state.v.lin.w = eskf_filter.GetNominalVel();
  this->state.v.ang.w = this->state.q.toRotationMatrix() * this->state.v.ang.b;
  this->state.v.lin.b = this->state.q.toRotationMatrix().inverse() * this->state.v.lin.w;
...
void dlio::OdomNode::updateState() {
  ...
  Sophus::SO3f SO3_R = Sophus::SO3<float>(qin.normalized().toRotationMatrix());
  Sophus::SE3<float> lidarSE3(SO3_R, pin);

  eskf_filter.ObserveSE3(lidarSE3, 0.05, 1.0 * M_PI / 180);
  this->state.q = Eigen::Quaternionf(eskf_filter.GetNominalSE3().so3().matrix());
  this->state.p = eskf_filter.GetNominalSE3().translation();
  this->state.v.lin.w = eskf_filter.GetNominalVel();
  this->state.v.ang.w = this->state.q.toRotationMatrix() * this->state.v.ang.b;
  this->state.v.lin.b = this->state.q.toRotationMatrix().inverse() * this->state.v.lin.w;
  this->state.b.gyro = eskf_filter.GetNominalBg();
  this->state.b.accel = eskf_filter.GetNominalBa();
...
void dlio::OdomNode::updateKeyframes() {
  ...
  if(this->force_no_update)
  {
    this->force_no_update = false;
    newKeyframe = false;
  }
  ...
  if (newKeyframe) {
    ...
    this->keyframe_normals.push_back(this->gicp.getSourceCovariances());
    this->keyframe_transformations.push_back(this->T_corr);
    // this->keyframe_transformations.push_back(this->T_discrepency);
