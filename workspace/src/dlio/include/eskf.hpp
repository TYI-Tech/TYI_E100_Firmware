...
namespace dlio
{
    ...
    public:
        ...
        using Vec3d = Eigen::Matrix<S, 3, 1>;
        using Vec6d = Eigen::Matrix<S, 6, 1>;
        using Vec18T = Eigen::Matrix<S, 18, 1>; // 18维向量类型
        using Mat3T = Eigen::Matrix<S, 3, 3>;   // 3x3矩阵类型
        using Mat6d = Eigen::Matrix<S, 6, 6>;
        ...
        struct Options
        {
            ...
            double gyro_var_ = 5e-2;      // 陀螺测量标准差
            double acce_var_ = 9e-1;      // 加计测量标准差
            double bias_gyro_var_ = 9e-4; // 陀螺零偏游走标准差
            double bias_acce_var_ = 2e-4; // 加计零偏游走标准差
        ...
        void SetInitialConditions(Options options, const VecT &init_bg, const VecT &init_ba,
                                  const VecT &gravity = VecT(0, 0, -9.80655))
        {
            BuildNoise(options);
            options_ = options;
            bg_ = init_bg;
            ba_ = init_ba;
            g_ = gravity;
            cov_ = Mat18T::Identity() * 1e-3;
        }

        /// 使用IMU递推
        bool Predict(const ImuMeas &imu);
         ...
         * @return
         */
        bool ObserveSE3(const SE3 &pose, double trans_noise = 0.1, double ang_noise = 1.0 * M_PI / 180);
        ...
        VecT GetNominalBg() const { return bg_; }
        VecT GetNominalBa() const { return ba_; }
    ...
    template <typename S>
    bool ESKF<S>::Predict(const ImuMeas &imu)
    {
        assert(imu.stamp >= current_time_);

        double dt = imu.stamp - current_time_;
        // INFO("imu time {0}  last time {1}", imu->time, current_time_);
        if (dt > (5 * options_.imu_dt_) || dt < 0)
        {
            // 时间间隔不对，可能是第一个IMU数据，没有历史信息
            // LOG(INFO) << "skip this imu because dt_ = " << dt;
            current_time_ = imu.stamp;
            return false;
        }

        // nominal state 递推
        VecT new_p = p_ + v_ * dt + 0.5 * (R_ * (imu.lin_accel   - ba_)) * dt * dt + 0.5 * g_ * dt * dt;
        VecT new_v = v_ + R_ * (imu.lin_accel   - ba_) * dt + g_ * dt;
        SO3 new_R = R_ * SO3::exp((imu.ang_vel   - bg_) * dt);
        ...
        F.template block<3, 3>(3, 6) = -R_.matrix() * SO3::hat(imu.lin_accel   - ba_ ) * dt; // v对theta
        F.template block<3, 3>(3, 12) = -R_.matrix() * dt;                            // v 对 ba
        F.template block<3, 3>(3, 15) = Mat3T::Identity() * dt;                       // v 对 g
        F.template block<3, 3>(6, 6) = SO3::exp(-(imu.ang_vel  - bg_) * dt).matrix();   // theta 对 theta
        F.template block<3, 3>(6, 9) = -Mat3T::Identity() * dt;                       // theta 对 bg

        // mean and cov prediction
        dx_ = F * dx_; // 这行其实没必要算，dx_在重置之后应该为零，因此这步可以跳过，但F需要参与Cov部分计算，所以保留
        // INFO("first COV {0} {1} {2}", cov_(0,0), cov_(0,1), cov_(0,2));
        cov_ = (F * cov_ * F.transpose() + Q_);
        // INFO("Q_ {0} {1} {2}", Q_(0,0), Q_(0,1), Q_(0,2));
        // INFO("COV {0} {1} {2}", cov_(0,0), cov_(0,1), cov_(0,2));
        // INFO("F {0} {1} {2}", F(0,0), F(0,1), F(0,2));
        // INFO("dx_ {0} {1} {2}", dx_(0), dx_(1), dx_(2));
        current_time_ = imu.stamp;
        return true;
    }
    ...
    bool ESKF<S>::ObserveSE3(const SE3 &pose, double trans_noise, double ang_noise)
    {
        ...
        // if (innov.tail(3).norm() > 10 * M_PI / 180)
        // {
        //     return false;
        // }
        /// 既有旋转，也有平移
        ...
        // INFO("debug {0} {1} {2}", dx_.template block<3, 1>(6, 0)(0), dx_.template block<3, 1>(6, 0)(2), dx_.template block<3, 1>(6, 0)(3));

        // cov_ = (Mat18T::Identity() - K * H) * cov_;
        // update this covariance using a different way
        cov_ = (Mat18T::Identity() - K * H) * cov_ * (Mat18T::Identity() - K * H).transpose() + K * V * K.transpose();
